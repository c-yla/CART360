...
/*
 * I had to change my RGB colors to 965 instead of 569.
 */
...
void recordWithDuration()
{
  ...
  // theAdnuster is 30 because it is present to have a little noise.
  int theAdjuster = 30;
  // This indicates that it will not store more than 16 notes which are MAX_NOTES
  if (countNotes < MAX_NOTES)
  {
    Serial.println(testNote);
    // testNote will read the value once the button is pressed down.
    testNote = analogRead(NOTE_IN_PIN);
    /*** STATE A::: *******************************************
...
void startUpTimer()
{
  //IMPLEMENT
 /*
  * This was taken from the demo i. This function will allow
  * the timer to reset.
  */
  startTime=millis(); 
...
{
  /*
     * This was taken from the demo i.
     * Time recording of the time passed minus the initial start time.
     */
...
void playCurrentNote()
{
    //IMPLEMENT
    /*
     * Tone will play according to the note being pressed in RecordDuration 
     * while it will depend on how the photocell is reacting. The Duration is 
     * added at the end in the ().
     */
...
void updateArraysWithNoteAndTimings()
{
 //IMPLEMENT
 /*
  * This function is about updating information.
  * In order for the sound to update,note[countNotes] is adding the frequency 
  * values from the photocell. Countnotes +++ will enable the countnotes to update.
  * 
  */
 notes[countNotes] = activeFrequency + averageOffsetFreq;
   //This indicates that we're updating the time that has passed for each note.
   durations[countNotes] = timePassed;
   countNotes++;
  //This will reset for each note.
  timePassed = 0;
...
{
   /* This is a way to filter noisy date by averaging.
    * With the numbers of measurements we include in the average,
    * the more noise will be removed.
    * Running average filter function was used here to filter the amount of time.
    * This was taken from the power point.
    * Return averageVal = makes the code ouput into the arduino.
   */
 //IMPLEMENT
...
{
  //IMPLEMENT
  /* When this void is called, the for loop will count how long it has been pressed using the duration delay.
   *  It will play it back with the delay.
   */
    for (int i = 0; i <= countNotes; i++) {
...
void play()
{
  ...
  for(int i=0; i<countNotes;i++)
  {
    /* https://www.arduino.cc/en/Reference/Tone */
    tone(BUZZER_PIN, notes[i], duration);
    // The delay will wait for the tone to stop playing.
    delay(duration); 
    //This is verify if the button is pressed.
...
void record()
{
  ...
  if(countNotes < MAX_NOTES)
  {
    // This will add a note to the array if a value is present.
